...
void classInitialIp(int *ipIntAux) {
	cout << "Endereço IP inicial utilizavel:" << ipIntAux[0] << "." << ipIntAux[1] << "." << ipIntAux[2] << "." << ipIntAux[3]+1 << endl;
}
...
void strToInt(string ipStringAux,int *ipIntAux) {
	...
	int y=0,i=0;
	while(i!=16){
		if(ipStringAux[i]!='.' && ipStringAux[i]!=' '){
			auxStr+=ipStringAux[i];
			i++;
		}
		else{
			ipIntAux[y]=atoi(auxStr.c_str());
			auxStr=' ';
			y++;
			i++;
		}
